...
enum Args {
    ...
    Pack {
        ...
        #[arg(
            required = true,
            help = "the URL where to send requests at runtime to retrieve the decryption key"
        )]
        url: String,
    },
    ...
    #[command(
        name = "strip",
        about = "Remove the Playfair protection for a file (requires original _orig file)"
    )]
    Strip {
        #[arg(
            required = true,
            help = "the file to remove the protection for (normal filename, not _orig)"
        )]
        file: String,
    },
    ...
    #[command(
        name = "recover",
        about = "Recover a file protected by Playfair (doesn't require original _orig file)"
    )]
    Restore {
        #[arg(
            required = true,
            help = "the file to restore (normal filename, not _orig)"
        )]
        file: String,

        #[arg(
            required = false,
            help = "the decryption key; if you don't have it, Playfair will fetch it from the stored URL"
        )]
        key: Option<String>,
    },
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args {
        Args::Pack { file, key, url } => pack(&file, &key, &url),
        Args::Strip { file } => strip(&file),
        Args::Restore { file, key } => recover(&file, key),
    }
}

fn pack(file: &String, key: &str, url: &str) -> Result<(), Box<dyn Error>> {
    ...
    if path
        ...
        .ends_with("_orig")
    {
        println!("Filename ends with '_orig', which means it might've been generated by Playfair");
    ...
    if path.parent().unwrap().join(&original).exists() {
        ...
        println!(
            "This means that Playfair was already used on {}",
...
fn recover(file: &String, key: Option<String>) -> Result<(), Box<dyn Error>> {
    ...
    if original_path.exists() {
        ...
        println!(
            "If you want to forcefully recover from the protected file, please delete {}",
            original
        );
        exit(7);
    }
